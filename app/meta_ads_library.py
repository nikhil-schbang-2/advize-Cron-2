...
def download_image(url, filename):
    ...
    try:
        ...
        try:
            s3.head_object(Bucket=COMPETITOR_BUCKET_NAME, Key=s3_key_with_extension)
            already_exists_image = s3_key_with_extension
        except ClientError as e:
            print(
                f"An S3 error occurred while checking for video object existence with key {s3_key_with_extension}: {e}"
            )
        ...
        if db_update_status:
            ...
            for i, ad in enumerate(cleaned_ads):
                if ad["creative_type"] == "Video" and ad["media_url"]:
                    ...
                    if video_s3_url := download_video(ad["media_url"], filename_hash):
                        video_count += 1
                        print(f"Video URL: {video_s3_url}")
                ...
                if ad["creative_type"] == "Image" and ad["media_url"]:
                    ...
                    is_exist = cursor.fetchone()
                    if is_exist:
                        print(f"Image already exists in the database: {filename_hash}")
                        continue

                    if image_s3_url := download_image(ad["media_url"], filename_hash):
                        image_count += 1
                        print(f"Image Url: {image_s3_url}")

                        query = """
                            INSERT INTO dim_image_creative_scraped (
                                image_hash,
                                page_id,
                                asset_link
                            ) VALUES (%s, %s, %s)
                        ...
                        if cursor:
                            cursor.close()
                            print(
                                f"Image URL inserted into the database: {image_s3_url}"
                            )
                        print(f"Image URL inserted into the database: {image_s3_url}")
