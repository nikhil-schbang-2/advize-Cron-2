...
INSIGHTS_BASE_URL = f"https://graph.facebook.com/v22.0/act_{META_AD_ACCOUNT_ID}/insights"
...
FACT_TABLE_BREAKDOWN_CONFIGS = [
    ...
    {'type': 'platform,placement', 'api_breakdowns': ['publisher_platform', 'platform_position']},
...
DIMENSION_MAPPING = {
    'account': {'table': 'dim_account', 'name_column': 'account_id', 'id_column': 'account_id'},
    'region': {'table': 'dim_region', 'name_column': 'region_name', 'id_column': 'region_id'},
    'age': {'table': 'dim_age_group', 'name_column': 'age_range', 'id_column': 'age_id'},
    'gender': {'table': 'dim_gender', 'name_column': 'gender_name', 'id_column': 'gender_id'},
    'publisher_platform': {'table': 'dim_platform', 'name_column': 'platform_name', 'id_column': 'platform_id'},
    'platform_position': {'table': 'dim_placement', 'name_column': 'placement_name', 'id_column': 'placement_id'},
    'impression_device': {'table': 'dim_impression_device', 'name_column': 'impression_device_name', 'id_column': 'impression_device_id'},
}
...
def upsert_date_dimension(conn, date_obj: date):
    ...
    try:
        with conn.cursor() as cur:
            ...
            cur.execute("SELECT date_id FROM dim_date WHERE date_value = %s", (date_value,))
            ...
            if existing_result:
                ...
                if new_result:
                    conn.commit()
                    print(f"Successfully upserted date dimension '{date_value}' into dim_date. ID: {new_result[0]}")
                    return new_result[0]
                else:
                    print(f"Error: Insert into dim_date for date '{date_value}' did not return an ID.")
                    conn.rollback()
                    return None
...
def fetch_page_with_retry(url, params, headers, attempt=1, max_retries=5, initial_delay=120, max_delay=3600):
    """Fetches a single page of data from a given URL with retry logic and increased delays."""
    delay = initial_delay * (2 ** (attempt - 1))
    delay = min(delay, max_delay)

    try:
        print(f"Fetching page (Attempt {attempt}/{max_retries}). Retrying in {delay} seconds if needed...")
        ...
        if e.response.status_code == 429 or (e.response.status_code == 400 and "too many calls" in e.response.text.lower()):
            print("Rate limit hit.")
        if 400 <= e.response.status_code < 500 and e.response.status_code != 429 and not ("too many calls" in e.response.text.lower()):
    ...
    if attempt < max_retries:
        ...
        time.sleep(delay)
        return fetch_page_with_retry(url, params, headers, attempt + 1, max_retries, initial_delay, max_delay)
    else:
        print(f"Max retries reached. Failed to fetch page after multiple attempts. Last error: {last_error}")
        return {"error": f"Failed to fetch page after {max_retries} attempts. Last error: {last_error}"}


def fetch_ad_account_insights_with_breakdown(ad_account_id, fields, breakdowns, time_range):
    """
    ...
    print(f"Fetching insights for Ad Account {ad_account_id}, Breakdown: {breakdowns if breakdowns else 'None'}")

    while next_page_url:
        response_data = fetch_page_with_retry(next_page_url, current_params, HEADERS)

        if response_data and "error" in response_data:
            print(f"Error fetching insights for Ad Account {ad_account_id} with breakdown {breakdowns if breakdowns else 'None'}: {response_data['error']}")
            return None
        ...
        if response_data and "data" in response_data:
            ...
            if paging_info and "next" in paging_info:
             ...
             print(f"Warning: API response missing 'data' field for Ad Account {ad_account_id}, breakdown {breakdowns if breakdowns else 'None'}.")
             next_page_url = None
...
def process_and_insert_insights(insights_data, time_window_key, breakdown_config, db_conn):
    """
    ...
    print(f"Processing {len(insights_data)} insights rows for breakdown: '{breakdown_config['type']}'...")
    ...
    for row in insights_data:
        ...
        try:
            ...
            if not hierarchy_data:
                print(f"Warning: Could not find hierarchy data for ad_id {ad_id} by joining dimension tables. Skipping row.")
                continue
            ...
            link_click = sum([int(action.get('value', 0)) for action in actions if action.get('action_type') == 'link_click'])
            landing_page_view = sum([int(action.get('value', 0)) for action in actions if action.get('action_type') == 'landing_page_view'])
            purchase = sum([int(action.get('value', 0)) for action in actions if action.get('action_type') == 'omni_purchase'])
            lead = sum([int(action.get('value', 0)) for action in actions if action.get('action_type') == 'lead'])
            post_reaction = sum([int(action.get('value', 0)) for action in actions if action.get('action_type') == 'react'])
            post_shares = sum([int(action.get('value', 0)) for action in actions if action.get('action_type') == 'post'])
            post_save = sum([int(action.get('value', 0)) for action in actions if action.get('action_type') == 'onsite_conversion.post_save'])
            video_plays_3s = sum([int(action.get('value', 0)) for action in actions if action.get('action_type') == 'video_view'])
            thruplays = sum([int(action.get('value', 0)) for action in actions if action.get('action_type') == 'video_thruplay_watched'])
            # print(f"Actions Metrics: Link Click = {link_click}, ...")


            action_values = row.get('action_values', [])
            purchase_revenue = sum([float(av.get('value', 0)) for av in action_values if av.get('action_type') == 'omni_purchase'])
            lead_revenue = sum([float(av.get('value', 0)) for av in action_values if av.get('action_type') == 'lead'])
            ...
            if breakdown_type == 'account':
                 ...
                 account_dim_id = upsert_dimension(db_conn, mapping['table'], mapping['name_column'], account_name, mapping['id_column'])

            elif breakdown_type == 'region' and 'region' in row:
                 region_name = row.get('region')
                 mapping = DIMENSION_MAPPING['region']
                 region_id = upsert_dimension(db_conn, mapping['table'], mapping['name_column'], region_name, mapping['id_column'])
                 if region_id is None:
                     print(f"Warning: Skipping row for ad {ad_id} due to missing region_id for breakdown '{breakdown_type}'. Region name: '{region_name}'.")
                     continue
                 ...
                 age_id = upsert_dimension(db_conn, mapping_age['table'], mapping_age['name_column'], age_range, mapping_age['id_column'])
                 gender_id = upsert_dimension(db_conn, mapping_gender['table'], mapping_gender['name_column'], gender_name, mapping_gender['id_column'])
                 if age_id is None or gender_id is None:
                      print(f"Warning: Skipping row for ad {ad_id} due to missing age_id ({age_id}) or gender_id ({gender_id}) for breakdown '{breakdown_type}'. Age: '{age_range}', Gender: '{gender_name}'.")
                      continue

            elif breakdown_type == 'platform,placement' and 'publisher_platform' in row and 'platform_position' in row:
                 ...
                 platform_id = upsert_dimension(db_conn, mapping_platform['table'], mapping_platform['name_column'], platform_name, mapping_platform['id_column'])
                 placement_id = upsert_dimension(db_conn, mapping_placement['table'], mapping_placement['name_column'], placement_name, mapping_placement['id_column'])
                 if platform_id is None or placement_id is None:
                      print(f"Warning: Skipping row for ad {ad_id} due to missing platform_id ({platform_id}) or placement_id ({placement_id}) for breakdown '{breakdown_type}'. Platform: '{platform_name}', Placement: '{placement_name}'.")
                      continue
                 ...
                 impression_device_id = upsert_dimension(db_conn, mapping['table'], mapping['name_column'], impression_device_name, mapping['id_column'])
                 if impression_device_id is None:
                      print(f"Warning: Skipping row for ad {ad_id} due to missing impression_device_id for breakdown '{breakdown_type}'. Impression Device: '{impression_device_name}'.")
                      continue
            ...
            data_tuple = (
                time_window_key, start_date_obj, end_date_obj, ad_id, account_id, adset_id, campaign_id,
                region_id, age_id, gender_id, platform_id, placement_id, impression_device_id,
                breakdown_type,
                amount_spent, video_plays_3s, impressions, reach, thruplays,
                link_click, landing_page_view, purchase, lead, post_reaction, post_shares, post_save,
    ...
    if rows_to_insert:
        try:
            ...
            db_conn.commit()
            print(f"Successfully inserted/updated {len(rows_to_insert)} rows for breakdown '{breakdown_config['type']}'.")
        except Exception as e:
            db_conn.rollback()
...
if __name__ == '__main__':
    ...
    if db_connection:
        ...
        try:
            ...
            print("Could not fetch ad IDs from dim_ad. Please ensure dim_ad exists and is populated.")

        if not valid_ad_ids:
            print("No ads found in dim_ad table or failed to fetch. Cannot populate fact table as hierarchy data cannot be retrieved. Exiting.")
            db_connection.close()
            exit()
        ...
        for breakdown_config in FACT_TABLE_BREAKDOWN_CONFIGS:
            ...
            if insights_data is not None:
                for time_window_key in FACT_TABLE_TIME_WINDOWS:
                    process_and_insert_insights(insights_data, time_window_key, breakdown_config, db_connection)

            else:
                print(f"  Skipping processing for breakdown '{breakdown_config['type']}' due to fetch errors.")
